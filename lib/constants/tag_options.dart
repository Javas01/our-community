import 'package:flutter/material.dart';

final tagOptions = {
  'Events': Colors.redAccent,
  'School': Colors.blueAccent,
  'Masjid': Colors.deepPurpleAccent,
  'Skillshare': Colors.greenAccent,
  'Business': Colors.orangeAccent,
  'Conference': Colors.blueAccent,
  'Study': Colors.deepPurpleAccent,
  'Industry': Colors.greenAccent,
  'Other': Colors.cyanAccent,
};

final businessOptions = {
  'Lawyer': Colors.redAccent,
  'Restaurant': Colors.blueAccent,
  'Mechanic': Colors.deepPurpleAccent,
  'Grocery': Colors.greenAccent,
  'Fast Food': Colors.orangeAccent,
  'Near Masjid': Colors.tealAccent,
  'Other': Colors.cyanAccent,
};

final eventOptions = {
  'Quran': Colors.redAccent,
  'Sports': Colors.blueAccent,
  'Recreation': Colors.deepPurpleAccent,
  'Meet': Colors.greenAccent,
  'Other': Colors.cyanAccent,
};

final tagOptionsList = <Map<String, MaterialAccentColor>>[
  {'Events': Colors.redAccent},
  {'School': Colors.blueAccent},
  {'Masjid': Colors.deepPurpleAccent},
  {'Skillshare': Colors.greenAccent},
  {'Business': Colors.orangeAccent},
  {'Other': Colors.cyanAccent},
];

final businessOptionsList = <Map<String, MaterialAccentColor>>[
  {'Lawyer': Colors.redAccent},
  {'Restaurant': Colors.blueAccent},
  {'Mechanic': Colors.deepPurpleAccent},
  {'Grocery': Colors.greenAccent},
  {'Fast Food': Colors.orangeAccent},
  {'Near Masjid': Colors.tealAccent},
  {'Other': Colors.cyanAccent},
];

final eventOptionsList = <Map<String, MaterialAccentColor>>[
  {'Quran': Colors.redAccent},
  {'Sports': Colors.blueAccent},
  {'Recreation': Colors.deepPurpleAccent},
  {'Meet': Colors.greenAccent},
  {'Other': Colors.cyanAccent},
];

final conferenceTagOptionsList = <Map<String, MaterialAccentColor>>[
  {'Events': Colors.redAccent},
  {'Conference': Colors.blueAccent},
  {'Study': Colors.deepPurpleAccent},
  {'Industry': Colors.greenAccent},
  {'Skillshare': Colors.orangeAccent},
  {'Other': Colors.cyanAccent},
];
